      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
        ...
        <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }}
          ...
          <div style={{ padding: "24px 28px", borderBottom: "1px solid #E2E8F0", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
            ...
            <button type="button" onClick={onClose} aria-label="Close resume curation" style={{ padding: 8, background: "#F8FAFC", border: "1px solid #E2E8F0", borderRadius: 8, cursor: "pointer" }}>
          ...
          <div style={{ padding: "20px 28px", borderTop: "1px solid #E2E8F0", display: "flex", gap: 10 }}>
            <button type="button" onClick={() => setSaved(true)} className={saved ? "btn-outline" : "btn-primary"} style={{ flex: 1, justifyContent: "center" }}>
...
function BridgeBot({ profile }: { profile: Record<string, unknown> }) {
  ...
  const send = useCallback(async (text: string) => {
    ...
    setThinking(true)
    try {
      const res = await fetch(`${API_BASE_URL}/chat/bridge-bot`, {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  return (
    <>
      <motion.button type="button" aria-label={open ? "Close BridgeBot" : "Open BridgeBot"} aria-expanded={open} whileHover={{ scale: 1.06 }} whileTap={{ scale: 0.95 }} onClick={() => setOpen(o => !o)}
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, scale: 0.92, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.92, y: 20 }}
            ...
            role="dialog"
            aria-label="BridgeBot resource advisor"
            className="bridgebot-panel"
            ...
            <div style={{ padding: "12px 14px", borderTop: "1px solid #E2E8F0", display: "flex", gap: 8 }}>
              <input value={input} onChange={e => setInput(e.target.value)}
                aria-label="Ask BridgeBot about your profile"
                onKeyDown={e => e.key === "Enter" && !e.shiftKey && send(input)}
                placeholder="Ask about your eligibility..." className="input" style={{ fontSize: 13, padding: "9px 12px" }} />
              <button type="button" aria-label="Send message" onClick={() => send(input)} disabled={!input.trim() || thinking}
...
function ResourceCard({ resource, onCurate, isSaved, onSave, now }: {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card p-6"
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {isJob && (
          <button type="button" onClick={() => onCurate(resource)} className="btn-outline" style={{ flex: 1, justifyContent: "center", fontSize: 13 }}>
        ...
        <button type="button" onClick={() => onSave(resource.id)} aria-label={isSaved ? `Remove ${resource.name} from saved resources` : `Save ${resource.name}`}
...
export default function DashboardPage() {
  ...
  const seedDemoProfile = async () => {
    ...
    try {
      const matchRes = await fetch(`${API_BASE_URL}/match/ledger`, {
        method: "POST",
      ...
      <section className="mx-auto flex min-h-screen w-full max-w-3xl items-center justify-center px-6 py-24 text-center">
        <div className="w-full rounded-lg border border-slate-200 bg-white p-8 shadow-sm">
          ...
          <div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row">
            ...
            <button type="button" onClick={seedDemoProfile} disabled={demoLoading} className="btn-outline justify-center disabled:cursor-not-allowed disabled:opacity-60">
    ...
    <main style={{ minHeight: "100vh", background: "#F8FAFC", paddingBottom: 80 }}>
      ...
      <div className="max-w-6xl mx-auto px-6" style={{ paddingTop: 88 }}>
        ...
        <div className="tab-bar mb-6" style={{ overflowX: "auto" }}>
          {CATEGORIES.map(cat => (
            <button key={cat} type="button" onClick={() => setActiveTab(cat)} className={`tab-item ${activeTab === cat ? "active" : ""}`}>
              {cat}
              {data && cat !== "All" && (
            ...
            : filteredMatches.length > 0
              ...
              : (
                <div style={{ gridColumn: "1/-1", textAlign: "center", padding: "60px 20px" }}>
                  ...
                  <button type="button" onClick={() => setActiveTab("All")} className="btn-outline" style={{ marginTop: 16, display: "inline-flex" }}>
